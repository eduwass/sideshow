...
test("html surfaces inline local assets so opaque sandboxes do not need auth cookies", async () => {
  const app = makeApp();
  const uploaded = await app.request(
    "/api/assets",
    json({ data: b64([137, 80, 78, 71]), contentType: "image/png" }),
  );
  ...
  const published = await app.request(
    ...
    json({
      ...
      title: "sandboxed image",
      surfaces: [{ kind: "html", html: `<img src="/a/${asset.id}">` }],
    }),
  );
  const post = (await published.json()) as any;

  const rendered = await app.request(`/s/${post.id}?part=0&ver=1`);
  assert.equal(rendered.status, 200);
  assert.match(await rendered.text(), /src="data:image\/png;base64,iVBORw=="/);
});
